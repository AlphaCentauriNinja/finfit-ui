...
export default function AddBullionModal({ isOpen, onClose, onCreated }: Props) {
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        if (!Number.isFinite(parsedPurchaseValue) || parsedPurchaseValue <= 0) {
            setFormError(`Value must be greater than 0 in ${preferredCurrency}.`)
            return
        }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-3xl overflow-hidden rounded-2xl border border-white/10 bg-[#0f172a] shadow-xl animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between border-b border-white/10 p-6">
                    <div>
                        <h2 className="text-xl font-bold text-white">Add Bullion</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-5 p-6">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        />
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-white/80">{`Value (${preferredCurrency})`}</label>
                            <input
                                type="number"
                                required
                                min="0"
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-white/65">
                        The selected catalog item IDs are stored on the holding as references, while display fields are snapshotted for the current UI. Purchase value is recorded in your current settings currency: {preferredCurrency}.
                    </div>
                    ...
                    <div className="flex gap-3 pt-2">
                        <button
                            ...
                            onClick={handleClose}
                            disabled={isSaving}
                            className="flex-1 rounded-xl border border-rose-500/35 px-4 py-3 text-sm font-semibold text-rose-300 transition-colors hover:bg-rose-500/10 disabled:opacity-60"
                        >
                            Cancel
                        </button>
                        <button
                        ...
                        >
                            <span className="inline-flex items-center justify-center gap-2">
                                ...
                                {isSaving ? 'Saving...' : isCatalogLoading ? 'Loading Catalog...' : 'Add Bullion'}
                            </span>
                        </button>
                    </div>
                </form>
